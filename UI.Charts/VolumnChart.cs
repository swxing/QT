...
namespace QT.UI.Charts
{
   public class VolumnChart:FrameworkElement
   {
      ...
      protected override void OnRender(DrawingContext drawingContext)
      {
         ...
         drawingContext.DrawRectangle(Brushes.Transparent, null, rect);          //draw border
         ...
         var bars = BarSet.GetBarSet(this._state.Symbol, this._state.Interval);
         ...
         long volumnMax = selBars.Max(item => item.Volume);            //選出最大的值。


         var range = bars.Bars.Where(temp => temp.TimeStamp > selBars[0].TimeStamp.AddYears(-1)
         && temp.TimeStamp < selBars[selBars.Count-1].TimeStamp);
         ...
         {
            //@ 算出今天成交量的Rect的兩個點。
            index = selBars.IndexOf(item);                      //WPF中，左上角為原點。
